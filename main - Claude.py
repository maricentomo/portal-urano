...
# ========= Helpers Claude =========
def claude_messages_from_chat(history: Optional[List[ChatMessage]]) -> List[Dict]:
    messages: List[Dict] = []
...
def claude_text(system_text: str, messages: List[Dict], max_tokens: int = 12000) -> str:
    if not anthropic_client:
        raise HTTPException(
            status_code=503,
            detail="Serviço Claude não configurado. Configure ANTHROPIC_API_KEY.",
        )
...
""".strip()
            messages.append({"role": "user", "content": contexto_mapa})
            messages.append({"role": "assistant", "content": "Dados do mapa recebidos. Pronto para análise."})

        # Histórico
        messages.extend(claude_messages_from_chat(request.history))

        # Mensagem atual
        messages.append({"role": "user", "content": request.message})

        text = claude_text(INSTRUCOES_SISTEMA, messages, max_tokens=12000)
...
async def health_check():
    ...
    return {
        ...
        # "gemini_configured": GOOGLE_API_KEY is not None,  # (COMENTADO)
        "database": "connected",
    }
...
async def send_message(
    ...
    try:
        history = [
            ChatMessage(role=msg.role, content=msg.content) for msg in messages_db[:-1]
        ]

        if not anthropic_client:
            raise HTTPException(status_code=503, detail="Claude não configurado (ANTHROPIC_API_KEY).")

        claude_msgs: List[Dict] = []

        if map_data:
            contexto_mapa = "DADOS DO MAPA:\n" + json.dumps(map_data, ensure_ascii=False, indent=2)
            claude_msgs.append({"role": "user", "content": contexto_mapa})
            claude_msgs.append({"role": "assistant", "content": "Dados recebidos."})

        for msg in history:
            role = "assistant" if msg.role == "assistant" else "user"
            claude_msgs.append({"role": role, "content": msg.content})

        claude_msgs.append({"role": "user", "content": message_data.message})

        answer = claude_text(INSTRUCOES_SISTEMA, claude_msgs, max_tokens=12000)
...
async def generate_report_endpoint(request: ReportRequest):
    ...
    try:
        ...
        # Claude: prompt vai como user, e o system é o INSTRUCOES_SISTEMA
        messages = [{"role": "user", "content": prompt}]
        analysis_text = claude_text(INSTRUCOES_SISTEMA, messages, max_tokens=12000)
